...
import { debounceTime, map, mergeAll, mergeMap, switchMap, tap } from "rxjs/operators";
...
input$.pipe(
    ...
    // tap(console.log),
    mergeMap<string, Observable<GitUsersRespond>>(texto => {
...
input$.pipe(
    ...
    switchMap(texto => ajax.getJSON(url + texto)) // las peticion anteror se cancela. ver RED = consola
